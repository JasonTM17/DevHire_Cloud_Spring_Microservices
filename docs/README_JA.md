...
v0.6 implementation は Stitch project `projects/5421325194779586117` に沿っています。Primary screenshots は raw UUID、`UNKNOWN`、loading-only state、smoke label、offline banner、fallback banner を含まないように検証されます。stacked v0.6.7 では Skill Assessment を flagship product feature に拡張し、candidate Assessment Studio、deterministic static scoring、attempt metadata、code hash、rubric versioning、employer review dossier、admin assessment health を追加しています。sandbox execution は後続の isolated-worker phase に残しています。
