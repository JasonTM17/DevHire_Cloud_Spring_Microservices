...
| v0.6 Stitch app | PR #43 は green。stacked v0.6.7 で code assessment を candidate grading、employer review、admin health の flagship workflow に拡張 |
...
v0.6 implementation は Stitch project `projects/5421325194779586117` に沿っています。Primary screenshots は raw UUID、`UNKNOWN`、loading-only state、smoke label、offline banner、fallback banner を含まないように検証されます。stacked v0.6.7 では Code Assessment Studio を flagship product feature に拡張し、candidate coding workspace、visible runner cases、hidden server-side scoring、deterministic rubric scoring、integrity/similarity posture、redacted list/detail boundaries、attempt metadata、code hash、rubric versioning、employer review dossier、admin assessment health を追加しています。v0.7 boundary では internal `assessment-runner-service` と Judge0-compatible adapter を追加し、domain ownership は `application-service` に残します。
