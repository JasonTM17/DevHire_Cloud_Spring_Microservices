...
v0.6 implementation は Stitch project `projects/5421325194779586117` に沿っています。Primary screenshots は raw UUID、`UNKNOWN`、loading-only state、smoke label、offline banner、fallback banner を含まないように検証されます。v0.6.4 turns Skill Assessment into a safe static code grading workflow; sandbox execution is reserved for a later isolated-worker phase.
