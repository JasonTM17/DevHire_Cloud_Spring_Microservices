...
| v0.6 Stitch app | PR #43 は green。stacked v0.6.6 で code assessment grading、employer review filters、submission history を追加 |
...
v0.6 implementation は Stitch project `projects/5421325194779586117` に沿っています。Primary screenshots は raw UUID、`UNKNOWN`、loading-only state、smoke label、offline banner、fallback banner を含まないように検証されます。stacked v0.6.6 では Skill Assessment を safe static code grading workflow に拡張し、candidate submission history、employer review filters、admin health evidence を追加しています。sandbox execution は後続の isolated-worker phase に残しています。
