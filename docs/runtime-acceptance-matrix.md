...
| Code assessment grading flow | `code-assessment-smoke.ps1` | employer assigns an assessment, candidate runs visible/custom Java cases, candidate submits hidden server-side scoring, hidden payloads stay redacted, employer reviews the result, and admin summary reports assessment health | candidate studio, assignment, scoring, redaction, review, or summary contract is broken | `application-service`, `assessment-runner-service`, `api-gateway` |
...
| Runtime domain metrics | `runtime-observability-smoke.ps1` | custom recruitment, outbox, email, audit, search, AI, and assessment-runner metrics exist with seeded samples or scrapeable gauges | observability is only infrastructure-level, not domain-runtime proof | core services and observability |
| Gateway route metrics | `runtime-observability-smoke.ps1` | Gateway request count, route latency, and rate-limit metrics are present after API traffic | Gateway traffic control cannot be tied to route-level SLOs | `api-gateway`, Redis rate limiter |
| Code runner observability | `runtime-observability-smoke.ps1` | runner queue depth, fail-closed state, Judge0 configured gauge, sandbox failures, request counters, and latency histograms are scrapeable | code grading failures cannot be detected before they affect candidates | `assessment-runner-service`, Prometheus |
